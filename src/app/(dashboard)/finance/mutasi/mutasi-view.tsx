...
export type Mutation = {
  ...
  method: string | null; refType: string | null; ref: string; brand: string; desc: string; isTransfer: boolean; createdAt: string | null;
};
...
export function MutasiView({ mutations, accounts }: { mutations: Mutation[]; accounts: AccountOpt[] }) {
  ...
  const brandOpts = useMemo(() => Array.from(new Set(mutations.map((m) => m.brand))).filter(Boolean).sort(), [mutations]);
  ...
  const list = useMemo(() => mutations
    ...
    .slice()
    .sort((a, b) => sortKey(b).localeCompare(sortKey(a))), [mutations, accId, brand, from, to, query]);
  ...
  function download() {
    const head = ["Tanggal", "Akun", "Brand", "Keterangan", "Referensi", "Metode", "Debit (Masuk)", "Kredit (Keluar)"];
    const lines = list.map((m) => [
      m.date ?? "", m.account, m.brand, m.desc, m.ref === "—" ? "" : m.ref, m.method ?? "",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        {(from || to || accId || brand || q) && <button onClick={() => { setFrom(""); setTo(""); setAccId(""); setBrand(""); setQ(""); }} className="h-10 rounded-xl border border-border px-3 text-sm font-bold hover:bg-muted">Reset</button>}
      </div>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="card p-5"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Total Debit (Masuk)</p><p className="mt-1 text-3xl font-black tabular-nums tracking-tight text-emerald-700">{formatIDR(totalDebit)}</p></div>
        <div className="card p-5"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Total Kredit (Keluar)</p><p className="mt-1 text-3xl font-black tabular-nums tracking-tight text-danger">{formatIDR(totalKredit)}</p></div>
        <div className="card p-5"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Selisih</p><p className="mt-1 text-3xl font-black tabular-nums tracking-tight">{formatIDR(totalDebit - totalKredit)}</p></div>
      </div>
          ...
          <table className="w-full min-w-[1040px] text-sm">
            ...
            <tbody>
              {list.map((m) => (
                <tr key={m.id} className="border-t border-border font-semibold hover:bg-muted/40">
                  ...
                  <td className="py-2.5 pr-3">
                    {m.brand === "Umum"
                      ? <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-bold text-muted-foreground">Umum</span>
                      : <span className="rounded-full bg-alice px-2 py-0.5 text-[11px] font-bold text-eerie">{m.brand}</span>}
                  </td>
                  <td className="py-2.5 pr-3">
              ...
              <tr className="border-t-2 border-border bg-muted/40 font-extrabold">
                <td className="py-2.5 pl-4 pr-3" colSpan={6}>Total ({list.length} transaksi)</td>
