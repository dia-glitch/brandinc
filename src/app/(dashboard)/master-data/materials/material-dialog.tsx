...
export function MaterialDialog({ material, categories }: { material?: MaterialData; categories: CatOpt[] }) {
  ...
  function close() { setOpen(false); setConfirmDelete(false); setError(null); }

  function submit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    startTransition(async () => {
      const input = { name, categoryId: categoryId || null, unit, is_active: active };
          ...
          <div className="w-full max-w-md rounded-2xl bg-surface p-6 shadow-soft">
            ...
            <form onSubmit={submit} className="space-y-4">
              {isEdit && material ? (
                ...
                <p className="text-xs font-medium text-muted-foreground">Kode dibuat otomatis (MTR-0001, …).</p>
              )}
