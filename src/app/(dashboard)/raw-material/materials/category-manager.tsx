...
export function CategoryManager({ categories }: { categories: MaterialCategory[] }) {
  ...
  function add() {
    ...
    startTransition(async () => {
      const res = await createMaterialCategory(newName, newCode);
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-eerie/40 p-4">
          <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-surface p-6 shadow-soft">
            ...
            <p className="mb-3 text-xs font-medium text-muted-foreground">Kode = prefix untuk kode material (mis. FB → FB-0001).</p>
            <div className="mb-4 flex gap-2">
              <input value={newName} onChange={(e) => setNewName(e.target.value)}
                className="h-11 flex-1 rounded-xl border border-border bg-background px-3.5 text-sm font-medium outline-none focus:border-primary/40" placeholder="Nama kategori" />
              <input value={newCode} onChange={(e) => setNewCode(e.target.value)}
                className="h-11 w-20 rounded-xl border border-border bg-background px-3 text-sm font-bold uppercase outline-none focus:border-primary/40" placeholder="Kode" />
              <Button type="button" size="sm" onClick={add} disabled={pending}><Plus className="h-4 w-4" /></Button>
            </div>
...
function Row({ category }: { category: MaterialCategory }) {
  ...
  const router = useRouter();
  const changed = name.trim() !== category.name || code.trim().toUpperCase() !== (category.code ?? "");
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input value={code} onChange={(e) => setCode(e.target.value)}
        className="h-10 w-16 rounded-xl border border-border bg-background px-2 text-sm font-bold uppercase outline-none focus:border-primary/40" placeholder="Kode" />
      {changed && (
        <button type="button" onClick={() => startTransition(async () => { await updateMaterialCategory(category.id, name, code); router.refresh(); })} disabled={pending}
