...
export default async function ProdInvoicePrintPage({ params }: { params: { id: string } }) {
  ...
  const { data: po } = await supabase
    .from("production_pos")
    .select("id,code,spk_id,brand_id,company_id,supplier_id,ppn_percent,invoice_no,invoice_date")
  ...
  if (!po.invoice_no) {
    return (
      <div className="mx-auto max-w-2xl p-8 text-center text-eerie">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Buka Production → PO Produksi → klik &quot;Buat Invoice&quot; (butuh Good final).</p>
      </div>
    );
  }

  const [lineRes, spkRes, brandRes, supRes, coRes] = await Promise.all([
    supabase.from("production_po_lines").select("sku,size,product_name,received_qty,unit_cost").eq("po_id", po.id).is("deleted_at", null),
  ...
  // Nilai invoice SELALU dari qty final good (received_qty).
  const lines = (lineRes.data ?? [])
    .map((l) => ({
      name: (l.product_name as string | null) ?? (l.sku as string) ?? "—",
    ...
    <div className="mx-auto max-w-3xl bg-white p-8 text-eerie">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Invoice Produksi · Tagihan Jasa Vendor</p>
        </div>
        <PrintButton />
      </div>
      ...
      <div className="mb-6 flex items-end justify-between border-t border-b border-border py-4">
        ...
        <div className="text-right text-sm">
          ...
          <p className="text-muted-foreground">SPK: {spkCode} · Brand: {brandName}</p>
          <p className="text-muted-foreground">Nilai sesuai Good final</p>
        </div>
      </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border text-left text-xs font-bold uppercase text-muted-foreground">
            ...
            <th className="py-2 text-right">Good Final</th>
      ...
      <p className="mt-6 text-xs text-muted-foreground">
        Tagihan jasa produksi atas PO {po.code} berdasarkan jumlah barang Good final yang diterima & lolos QC.
      </p>
    </div>
  );
}
